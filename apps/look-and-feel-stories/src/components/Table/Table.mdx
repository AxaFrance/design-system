...
- `Table.THead` - Table header with optional `color` prop ("gray")
...
- `Table.Th` - Table header cell with optional `onSort`, `onCheck`, and `checkboxPosition` props for sortable and selectable tables
...
Add `onCheck` prop to `Table.Th` to enable row selection. Use `checkboxPosition` to control checkbox placement.
