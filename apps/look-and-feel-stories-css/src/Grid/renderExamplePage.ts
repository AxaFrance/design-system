...
import { render as renderContact } from "../ContactRender";
...
const renderLayout = () => {
  ...
  const contact1 = renderContact(renderContactArgs);
  const contact2 = renderContact(renderContactArgs);
...
export const render = () => {
  ...
  return div([...renderDebugGrid(), header, main, footer]);
};
