...
Tweak `formTitle` and the `sections` array directly via Storybook controls.
...
- The section titles are rendered as `<h3>` for better screen reader support.
...
- **Type:** `Array<{ title?: string; items: Array<{ label: string; mode: string; anchor?: string }> }>` (required)
- **Description:**
  - Each section object can have:
    ...
    - `items`: **Array** — The list of step objects for this section.
      - Each item object can have:
        ...
        - `mode`: **string** — The visual state of the step (`locked`, `edited`, `validated`, `disabled`).
        - `anchor`: **string** (optional) — If provided, the label is rendered as a link to the anchor.
