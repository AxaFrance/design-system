...
To create a release candidate, you need to create a new branch from the `main` branch. The branch name should be `releases/X.Y.Z` where `X.Y.Z` is the version number you want.

```bash
git switch main
git pull
git switch -c releases/1.0.0
```
